...
* **Fine-grained.**  All activities are selected so that there are subtle differences between classes where the activity representation and discrimination is critical for performance, rather than the scene context or object detection.  Fine-grained activities are [tree structured](https://github.com/visym/cap/blob/main/classification/superlabel.pdf) by design.
